...
def run_cDNA_mines(coverage,ref,f_modified,output,kmer_models):
    ...
    seqs_df['chrom'] = list(map(lambda x: re.sub(r':[^:]*$','',x.split('>')[1]), seqs_df.index))
    d = dict()
    l1 = list()
    l2 = list()
    l3 = list()
    ...
    model_list = pd.read_csv(kmer_models, header=None, names=['file'])
    ...
    for kmer in set(df_final['kmer']):
        ...
        loaded_model = joblib.load(os.path.dirname(kmer_models)+'/'+fname)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ref',dest='ref',required=True, help='Reference Genome Fasta File')
    parser.add_argument('--coverage',dest='coverage', required=True, help='Tombo Text_Output Coverage File(*plus.bedgraph)')
    parser.add_argument('--fraction_modified',dest='fraction_modified', required=True, help='Tombo Text_Output Fraction_Modified File (*plus.wig)')
    ...
    output=args.output
    kmer_models=args.kmer_models
    run_cDNA_mines(coverage,ref,f_modified,output,kmer_models)
